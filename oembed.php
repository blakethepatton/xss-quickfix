...
class AutoEmbed {
	...
	public $embedHeight	= 500;
	public $embedWidth	= 850;

	public $providers = [
		// 'http:\/\/www\.shopstyle\.com\/browse.*'                   => ['http://api.embed.ly/v1/api/oembed', true],
		// 'http:\/\/www\.shopstyle\.com\/action\/apiVisitRetailer.*' => ['http://api.embed.ly/v1/api/oembed', true],
		// 'http:\/\/www\.shopstyle\.com\/action\/viewLook.*'         => ['http://api.embed.ly/v1/api/oembed', true],
		...
		'#https?://poll\.fm/.*#i'                             => ['http://polldaddy.com/oembed/', true],
	...
	*/
	public function parse($content) {
		return preg_replace_callback('|^\s*(https?://[^\s"]+)\s*$|im', [$this, 'autoembed_callback'], $content);
	}
	...
	*/
	public function fetch($provider, $url, $args = []) {
		...
		$provider = $this->add_query_arg('height', (int) $args['height'], $provider);
